...
const uiMarker = "9router-auto-router-ui:v5";
...
function controls(jsx, select, name, combo) {
...
function discoverUiStructure(source, target) {
  ...
  if (["_arInitialStrategy", "_arAvailableCombos", "_arStrategyOptions", "_arSave"].some((binding) => source.includes(binding))) fail(`Auto Router UI generated binding collision:\n  ${path.relative(appRoot, target)}`);
  return { options, optionReference, optionArray, card, cardStrategy, selector, jsx, select, modal, modalSave, modalCombo, name, models, validator, savingSetter, modalStateStatement, saveDeclarator, footer, parent, invocation, edited, onSave, update, updateDeclarator, refresh, close, strategies, combos, explanation, explanationItem };
}
function verifyPatchedUi(target, source = fs.readFileSync(target, "utf8")) {
  const ast = parseJavaScript(source, target), functions = functionDeclarations(ast);
  if (count(source, uiMarker) !== 1 || count(source, 'label:"Auto Router"') !== 2 || !source.includes("onSave:_arSave")) fail(`Patched UI integrity failed:\n  ${path.relative(appRoot, target)}`);
  ...
  if (card.some((fn) => hasAnyLiteral(fn, ["Auto Router Settings", "Easy target", "Hard target", "Advanced"]))) fail(`Patched UI Auto Router controls leaked into the combo card:\n  ${path.relative(appRoot, target)}`);
}
...
function applyUiPlan(source, plan, target) {
  ...
  const state = `const _arStrategyOptions=${plan.optionArray.slice(0,-1)},{value:"auto",label:"Auto Router"}],_arBounds={${serializedBounds}},_arNumber=(value,key,fallback)=>{let bounds=_arBounds[key],number=Number(value);return Number.isSafeInteger(number)&&number>=bounds[0]&&number<=bounds[1]?number:fallback},_arSavedConfig=_arInitialStrategy.autoRouter&&typeof _arInitialStrategy.autoRouter==="object"?_arInitialStrategy.autoRouter:{},_arInitialConfig={..._arSavedConfig,hardThreshold:_arNumber(_arSavedConfig.hardThreshold,"hardThreshold",${DEFAULTS.hardThreshold}),longContextChars:_arNumber(_arSavedConfig.longContextChars,"longContextChars",${DEFAULTS.longContextChars}),largeToolResultChars:_arNumber(_arSavedConfig.largeToolResultChars,"largeToolResultChars",${DEFAULTS.largeToolResultChars}),manyTools:_arNumber(_arSavedConfig.manyTools,"manyTools",${DEFAULTS.manyTools})},[_arStrategy,_arSetStrategy]=(0,${plan.name.react}.useState)(_arInitialStrategy.fallbackStrategy||"fallback"),[_arConfig,_arSetConfig]=(0,${plan.name.react}.useState)(_arInitialConfig),_arUpdate=(key,value)=>_arSetConfig(config=>({...config,[key]:value})),[_arHard,_arSetHard]=(0,${plan.name.react}.useState)(String(_arInitialConfig.hardThreshold)),[_arContext,_arSetContext]=(0,${plan.name.react}.useState)(String(_arInitialConfig.longContextChars)),[_arToolResult,_arSetToolResult]=(0,${plan.name.react}.useState)(String(_arInitialConfig.largeToolResultChars)),[_arTools,_arSetTools]=(0,${plan.name.react}.useState)(String(_arInitialConfig.manyTools));`;
  const save = `async()=>{if(!${plan.validator}(${plan.name.value}))return;${plan.savingSetter}(!0);try{let config={..._arConfig,hardThreshold:_arNumber(_arHard,"hardThreshold",${DEFAULTS.hardThreshold}),longContextChars:_arNumber(_arContext,"longContextChars",${DEFAULTS.longContextChars}),largeToolResultChars:_arNumber(_arToolResult,"largeToolResultChars",${DEFAULTS.largeToolResultChars}),manyTools:_arNumber(_arTools,"manyTools",${DEFAULTS.manyTools})};await ${plan.modalSave}({name:${plan.name.value}.trim(),models:${plan.models.value}},"auto"===_arStrategy?{fallbackStrategy:"auto",autoRouter:config}:{fallbackStrategy:_arStrategy})}finally{${plan.savingSetter}(!1)}}`;
  ...
  const replacements = [
    ...
    { start: plan.selector.start, end: plan.selector.end, text: `(0,${plan.jsx}.jsx)("span",{className:"text-xs text-text-muted",children:(${plan.optionReference}.find(option=>option.value===${plan.cardStrategy})||{}).label||${plan.cardStrategy}})` },
