...
function isSettingsStrategyWriter(node) {
  if (node?.type !== "ArrowFunctionExpression" || !node.async || node.params.length !== 1 || !identifier(node.params[0]) || node.body?.type !== "BlockStatement" || node.body.body.length !== 1) return false;
  const strategies = identifier(node.params[0]), persist = node.body.body[0]?.expression, [request, setStrategies] = persist?.expressions || [], options = request?.argument?.arguments?.[1], headers = objectProperty(options, "headers")?.value, body = objectProperty(options, "body")?.value, payload = body?.arguments?.[0];
  return node.body.body[0]?.type === "ExpressionStatement" && persist?.type === "SequenceExpression" && persist.expressions.length === 2 && request?.type === "AwaitExpression" && request.argument?.type === "CallExpression" && identifier(request.argument.callee) === "fetch" && request.argument.arguments.length === 2 && literal(request.argument.arguments[0], "/api/settings") && options?.type === "ObjectExpression" && options.properties.length === 3 && literal(objectProperty(options, "method")?.value, "PATCH") && headers?.type === "ObjectExpression" && headers.properties.length === 1 && literal(objectProperty(headers, "Content-Type")?.value, "application/json") && body?.type === "CallExpression" && memberProperty(body.callee) === "stringify" && identifier(body.callee.object) === "JSON" && body.arguments.length === 1 && payload?.type === "ObjectExpression" && payload.properties.length === 1 && identifier(objectProperty(payload, "comboStrategies")?.value) === strategies && setStrategies?.type === "CallExpression" && identifier(setStrategies.callee) && setStrategies.arguments.length === 1 && identifier(setStrategies.arguments[0]) === strategies;
}
function isStrategyUpdateHandler(node) {
  if (node?.type !== "ArrowFunctionExpression" || !node.async || node.params.length !== 2 || !node.params.every((param) => identifier(param)) || node.body?.type !== "BlockStatement" || node.body.body.length !== 1) return null;
  const [name, change] = node.params.map(identifier), attempt = node.body.body[0];
  if (attempt?.type !== "TryStatement" || attempt.finalizer || attempt.block.body.length !== 2 || attempt.handler?.body?.body.length !== 1 || !identifier(attempt.handler.param)) return null;
  const [initialize, effect] = attempt.block.body, [copy, merged] = initialize?.declarations || [];
  const member = (value, object, property, computed = false) => value?.type === "MemberExpression" && value.computed === computed && identifier(value.object) === object && (computed ? identifier(value.property) === property : value.property?.name === property);
  if (initialize?.type !== "VariableDeclaration" || initialize.declarations.length !== 2 || !identifier(copy?.id) || !identifier(merged?.id) || copy.init?.type !== "ObjectExpression" || copy.init.properties.length !== 1 || copy.init.properties[0]?.type !== "SpreadElement" || !identifier(copy.init.properties[0].argument) || merged.init?.type !== "ObjectExpression" || merged.init.properties.length !== 2 || merged.init.properties[0]?.type !== "SpreadElement" || merged.init.properties[0].argument?.type !== "LogicalExpression" || merged.init.properties[0].argument.operator !== "||" || !member(merged.init.properties[0].argument.left, identifier(copy.id), name, true) || merged.init.properties[1]?.type !== "SpreadElement" || identifier(merged.init.properties[1].argument) !== change) return null;
  const strategy = identifier(merged.id), strategies = identifier(copy.id), fallback = (value) => member(value, strategy, "fallbackStrategy") && !value.computed, effects = effect?.expression?.expressions || [];
  if (effect?.type !== "ExpressionStatement" || !((effects.length === 3 && effects[1]?.type === "AwaitExpression" && identifier(effects[1].argument?.callee) === "fetch") || (effects.length === 2 && effects[1]?.type === "AwaitExpression" && identifier(effects[1].argument?.callee))) || effects[0]?.type !== "ConditionalExpression" || effects[0].test?.type !== "LogicalExpression" || effects[0].test.operator !== "&&" || !fallback(effects[0].test.left) || effects[0].test.right?.type !== "BinaryExpression" || effects[0].test.right.operator !== "!==" || !literal(effects[0].test.right.left, "fallback") || !fallback(effects[0].test.right.right) || effects[0].consequent?.type !== "AssignmentExpression" || effects[0].consequent.operator !== "=" || !member(effects[0].consequent.left, strategies, name, true) || identifier(effects[0].consequent.right) !== strategy || effects[0].alternate?.type !== "UnaryExpression" || effects[0].alternate.operator !== "delete" || !member(effects[0].alternate.argument, strategies, name, true)) return null;
  const report = attempt.handler.body.body[0]?.expression;
  return report?.type === "CallExpression" && memberProperty(report.callee) === "log" && identifier(report.callee.object) === "console" && report.arguments.length === 2 && literal(report.arguments[0], "Error updating combo strategy:") && identifier(report.arguments[1]) === identifier(attempt.handler.param) ? { strategies, effects } : null;
}
...
function discoverUiStructure(source, target) {
  ...
  const strategyUpdateDeclarator = one(variableDeclarators(parent).filter(({ node }) => node.id?.name === strategyUpdate && isStrategyUpdateHandler(node.init)), "combo strategy update handler", target), strategyHandler = isStrategyUpdateHandler(strategyUpdateDeclarator.node.init), strategyEffects = strategyHandler.effects;
  if (strategyEffects.length === 3) {
    const request = strategyEffects[1], setStrategies = strategyEffects[2], requestOptions = request.argument.arguments[1], headers = objectProperty(requestOptions, "headers")?.value, body = objectProperty(requestOptions, "body")?.value, payload = body?.arguments?.[0];
    if (request.argument.arguments.length !== 2 || !literal(request.argument.arguments[0], "/api/settings") || requestOptions?.type !== "ObjectExpression" || requestOptions.properties.length !== 3 || !literal(objectProperty(requestOptions, "method")?.value, "PATCH") || headers?.type !== "ObjectExpression" || headers.properties.length !== 1 || !literal(objectProperty(headers, "Content-Type")?.value, "application/json") || body?.type !== "CallExpression" || memberProperty(body.callee) !== "stringify" || identifier(body.callee.object) !== "JSON" || body.arguments.length !== 1 || payload?.type !== "ObjectExpression" || payload.properties.length !== 1 || identifier(objectProperty(payload, "comboStrategies")?.value) !== strategyHandler.strategies || setStrategies?.type !== "CallExpression" || !identifier(setStrategies.callee) || setStrategies.arguments.length !== 1 || identifier(setStrategies.arguments[0]) !== strategyHandler.strategies) fail(`Auto Router UI strategy persistence handler is incompatible:\n  ${path.relative(appRoot, target)}`);
  } else one(variableDeclarators(parent).filter(({ node }) => node.id?.name === identifier(strategyEffects[1].argument.callee) && isSettingsStrategyWriter(node.init)), "combo strategy settings writer", target);
