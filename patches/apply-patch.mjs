...
const uiMarker = "9router-auto-router-ui:v8";
...
function discoverUiStructure(source, target) {
  ...
  if (onSetStrategy?.type !== "ArrowFunctionExpression" || onSetStrategy.params.length !== 1 || !cardCombo) fail(`Auto Router UI strategy persistence data flow is incompatible:\n  ${path.relative(appRoot, target)}`);
...
function applyUiPlan(source, plan, target) {
  ...
  const legacySetup = `const _arSavedConfig=_arInitialStrategy.autoRouter&&typeof _arInitialStrategy.autoRouter==="object"?_arInitialStrategy.autoRouter:{},_arLegacyTargets=typeof _arSavedConfig.easyTarget==="string"&&_arSavedConfig.easyTarget.trim()&&typeof _arSavedConfig.hardTarget==="string"&&_arSavedConfig.hardTarget.trim()&&_arSavedConfig.easyTarget.trim()!==_arSavedConfig.hardTarget.trim(),_arStoredModels=Array.isArray(${modelInitial})?${modelInitial}:[],_arEffectiveModels=_arLegacyTargets?[_arSavedConfig.easyTarget.trim(),_arSavedConfig.hardTarget.trim(),..._arStoredModels.filter(model=>typeof model!=="string"||(model.trim()!==_arSavedConfig.easyTarget.trim()&&model.trim()!==_arSavedConfig.hardTarget.trim()))]:null;`;
