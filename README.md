...
Classification is local and deterministic. It logs route, level, bounded score, and reason labels only; request text is never logged. Semantic terms use case-insensitive whole tokens and adjacent token phrases, not raw substrings: `reviewStatus`, `migrationPlan`, filenames such as `migration-plan.json`, and quoted labels do not count as task actions.
